...
dim ctable(8)=(&hA52A2A,&hFF00,&hAF00,&h6000,&h4000,0,&h80FF,&hFF00C0,&h404040)
    ...
    if object(i,0)then  ' shot exists
      ...
      if target>6 then  ' hit an enemy tank or an obstacle
        ...
        if target<9 then                  ' hit an enemy tank
          ...
          'if score>1 and object(6,0)=0 then v4=99:object(6,0)=1   ' stargate appears
          if score>level*2 and object(6,0)=0 then v4=99:object(6,0)=1   ' stargate appears
        endif
      endif
    endif
  next
...
sub DrawScene
  ...
  if h>239 and h<340 then blit write 1,3100-h*9.6,MM.VRES/10  ' Saturn
  ...
  for i=1to ASSETCOUNT
    ...
    if object(n,0)>0 then  ' exists/visible
      ...
      pixel 51+x*140/ARENASIZE,51-y*140/ARENASIZE,ctable(0+6*(object(n,6)=5)+(object(n,6)=4))  
      pixel 51,51,rgb(yellow)   ' player in center-redraw to ensure visible
